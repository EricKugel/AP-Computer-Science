...
public class ImageDisplay extends JPanel implements Scrollable {
  ...
  /**
   * Returns the unit increment for scrolling.
   * 
  ...
  /**
   * Checks if the viewport width is the source width.
   * 
  ...
  /**
   * Checks if the viewport height is the source height.
   * 
  ...
  public void paintComponent(Graphics g) {
    ...
    // checks if the current index is in the image
    if (currentX >= 0 && currentX < width && currentY >= 0 && currentY < height) {
      
      // checks that the start and end values are visible
      if (xStart < 0)
        xStart = 0;
      if (xEnd > maxX)
        xEnd = maxX;
